...
class PhaseObjectDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        diffraction_image = np.array(Image.open(diffraction_path).convert('L').resize((256, 256))) / 255.0
        ...
        image = np.array(Image.open(image_path).convert('L').resize((256, 256))) / 255.0
...
# Main program
image_dir = 'hm_image_starnet'  # Set your image directory
diffraction_dir = 'diffracted_images_startnet180'  # Set your diffraction directory
...
model = StarNet(base_dim=32).to(device)  # 将模型移动到指定设备
...
def train_network(model, train_dataloader, test_dataloader, criterion, optimizer, num_epochs):
    ...
    best_model_path = 'best_model_starnet(200).pth'  # 最佳模型权重保存路径
...
# Save model weights
torch.save(model.state_dict(), 'best_model_starnet(200).pth')
...
loss_data.to_excel('loss_values(200).xlsx', index=False)  # 保存到Excel文件
...
# # Main program
# image_dir = 'hm_image'  # Set your image directory
# diffraction_dir = 'diffracted_images_startnet'  # Set your diffraction directory
# dataset = PhaseObjectDataset(image_dir, diffraction_dir)
...
# train_losses = []
# test_losses = []
# best_test_loss = float('inf')  # 初始化最佳验证损失为无穷大
# best_model_state = None  # 初始化最佳模型状态

# # Training function
# def train_network(model, train_dataloader, test_dataloader, criterion, optimizer, num_epochs):
#     global best_test_loss, best_model_state
#     for epoch in range(num_epochs):
#         start_time = time.time()
#         model.train()
#         total_train_loss = 0.0
...
#         # Update best model if current epoch has lower test loss
#         if average_test_loss < best_test_loss:
#             best_test_loss = average_test_loss
...
# torch.save(best_model_state, 'best_model_starnet.pth')
