...
import time
import pandas as pd
from MobileNetV3 import MobileNetV3  # 确保导入的是支持512x512的StarNet版本
...
model = MobileNetV3().to(device)  # 使用支持512x512的StarNet版本
...
def train_network(model, train_dataloader, test_dataloader, criterion, optimizer, num_epochs):
    ...
    best_model_path = 'best_model_MobileNetV3(1).pth'  # 最佳模型权重保存路径
...
# Save model weights
torch.save(model.state_dict(), 'best_model_MobileNetV3(1).pth')
...
loss_data.to_excel('loss_values_MobileNetV3(1).xlsx', index=False)  # 保存到Excel文件
